...
**How it's built:**
- `index.html` — plain landing page. No founder list — it just tells visitors to use their personal link
- `founder.html` — the update page template. It reads `?id=<slug>` from the URL and loads **only that founder's data**, from `data/<slug>.json`. One template serves all 50 founders, each with their own distinct link, but no page ever loads another founder's information
- `data/` — 50 small JSON files, one per founder (name, company, milestone, metrics), pulled from `Top_50_Sprint_Statements_2026.xlsx`
...
- `founder-links.csv` — every founder's individual link in one file, for sending out (keep this one for yourself — don't upload it to the public site)

No server, database, or hosting account is required beyond a place to put static files and a Google account.

**A note on privacy, since founders shouldn't see each other's progress:** this is a static site with no login. Nothing lists the founders or their slugs anywhere public, and each page only ever loads its own founder's data — so there's no way to browse or stumble onto anyone else's page. But the security is "unguessable link," not a real login: anyone who has a specific founder's link (or guesses their slug, which is their name in lowercase-with-dashes) can open that one page. That matches what a link-based form like this can offer; if you need real per-founder authentication, that's a bigger build and I can help scope it if useful.
...
Upload these, and only these, to the root of the site (not nested in a subfolder): `index.html`, `founder.html`, `style.css`, `app.js`, `config.js`, and the whole `data/` folder. Leave out `Code.gs` and `founder-links.csv` — those aren't meant to be public.

**GitHub Pages**
1. Create a new GitHub repo and upload the files listed above, at the root of the repo.
2. In the repo, go to **Settings > Pages**, set Source to "Deploy from a branch," pick your branch and **/(root)**, and save.
3. Your site will be live at `https://<your-username>.github.io/<repo-name>/`.

**Netlify Drop**
1. Go to [app.netlify.com/drop](https://app.netlify.com/drop) and drag in a folder containing just the files listed above.
...
Open `founder-links.csv` — it has each founder's name, company, and the exact link to send them (as a mail-merge source, or just copy/paste). Keep this file for yourself; it isn't part of the uploaded site.
...
If the spreadsheet changes (names, milestones, etc.), re-run this against the source `.xlsx` to rebuild the `data/` folder and `founder-links.csv` (requires `openpyxl`):
...
for row in ws.iter_rows(min_row=3, values_only=True):
    ...
    while slug in seen:
        slug = f'{base}-{n}'; n += 1
    seen[slug] = True
    record = {'slug': slug, 'founderName': (founder or '').strip(),
              'companyName': (company or '').strip(),
              'milestone': (milestone or '').strip(),
              'metrics': (metrics or '').strip()}
    founders.append(record)
    json.dump(record, open(f'data/{slug}.json', 'w'), indent=2)

with open('founder-links.csv', 'w') as f:
    f.write('founder_name,company_name,slug,link_path\n')
    for r in founders:
        f.write(f'"{r["founderName"]}","{r["companyName"]}",{r["slug"]},founder.html?id={r["slug"]}\n')
```
