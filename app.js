...
async function initFounderPage() {
  ...
  wireWordCount('milestone-update', 'milestone-count');
  wireWordCount('metrics-update', 'metrics-count');
  wireWordCount('additional-update', 'additional-count');
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const additionalUpdate = document.getElementById('additional-update').value.trim();
    ...
    if (wordCount(milestoneUpdate) > WORD_LIMIT || wordCount(metricsUpdate) > WORD_LIMIT || wordCount(additionalUpdate) > WORD_LIMIT) {
      errorEl.textContent = `Keep each update to ${WORD_LIMIT} words or less.`;
      errorEl.dataset.visible = 'true';
      return;
    }
    if (!statusInput) {
...
function wireWordCount(fieldId, countId) {
  const field = document.getElementById(fieldId);
  const count = document.getElementById(countId);
  const limit = 150;
  const update = () => {
    const words = wordCount(field.value);
    count.textContent = `${words} / ${limit} words`;
    count.dataset.over = words > limit ? 'true' : 'false';
  };
